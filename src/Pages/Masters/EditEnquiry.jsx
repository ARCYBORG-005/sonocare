...
import { allocateEnquiryEmployee, checkDuplicateEnquiry } from './mockEnquiry';
import {
  mockStateData,
  mockDepartmentData,
...
const EditEnquiry = ({ enquiries, setEnquiries, employees = initialMockEmployees }) => {
  const navigate = useNavigate();
  const { id } = useParams();

  const targetEnquiry = useMemo(() => {
  ...
  useEffect(() => {
    if (targetEnquiry) {
      setFormData({
        enquiryId: targetEnquiry.enquiryId || '',
        customerName: targetEnquiry.customerName || '',
        contactPerson: targetEnquiry.contactPerson || '',
        customerType: targetEnquiry.customerType || 'Hospital',
        otherCustomerType: targetEnquiry.otherCustomerType || '',
        hospitalInstitution: targetEnquiry.hospitalInstitution || '',
        mobile: targetEnquiry.mobile || '',
        email: targetEnquiry.email || '',
        state: targetEnquiry.state || targetEnquiry.territory || 'Tamil Nadu',
        district: targetEnquiry.district || 'Chennai',
        city: targetEnquiry.city || 'Chennai',
        pincode: targetEnquiry.pincode || '',
        address: targetEnquiry.address || '',
        source: targetEnquiry.source || 'Website',
        sourceDetails: targetEnquiry.sourceDetails || '',
        campaignId: targetEnquiry.campaignId || '',
        campaignContactId: targetEnquiry.campaignContactId || '',
        conferenceName: targetEnquiry.conferenceName || '',
        referralName: targetEnquiry.referralName || '',
        referralOrg: targetEnquiry.referralOrg || '',
        referralEmail: targetEnquiry.referralEmail || '',
        referralMobile: targetEnquiry.referralMobile || '',
        referralTerritory: targetEnquiry.referralTerritory || '',
        priority: targetEnquiry.priority || 'Normal',
        department: targetEnquiry.department || 'Telecaller Team',
        productCategory: targetEnquiry.productCategory || 'Medical & Diagnostic Scanners',
        product: targetEnquiry.product || 'Sonocare HD Cardiac Probe Transducer',
        serviceInterested: targetEnquiry.serviceInterested || 'One-Time Purchase',
        subscriptionFrequency: targetEnquiry.subscriptionFrequency || 'Monthly',
        expectedTimeframe: targetEnquiry.expectedTimeframe || '1–3 Months',
        budget: targetEnquiry.budget || '',
        remarks: targetEnquiry.remarks || '',
        status: targetEnquiry.status || 'Pending',
        assignedEmployeeId: targetEnquiry.assignedEmployeeId || 'UNASSIGNED',
        assignedEmployeeName: targetEnquiry.assignedEmployeeName || 'Unassigned'
      });
    }
  }, [targetEnquiry]);
  ...
  // Evaluate Auto-Assignment if location or department changes
  useEffect(() => {
    const currentTerritory = formData.territory || formData.state;
    const targetTerritory = targetEnquiry ? (targetEnquiry.territory || targetEnquiry.state) : '';

    if (
      targetEnquiry &&
      (currentTerritory !== targetTerritory ||
        formData.district !== targetEnquiry.district ||
        formData.city !== targetEnquiry.city ||
        formData.department !== targetEnquiry.department ||
        formData.priority !== targetEnquiry.priority ||
        formData.source !== targetEnquiry.source)
    ) {
  ...
  }, [formData.territory, formData.state, formData.district, formData.city, formData.department, formData.priority, formData.source, targetEnquiry, employees, enquiries]);
  ...
  const executeUpdate = () => {
    setEnquiries((prev) =>
      prev.map((enq) =>
        enq.enquiryId === formData.enquiryId || String(enq.id) === String(id)
          ? {
            ...enq,
            ...formData,
            mobile: formData.mobile.trim(),
            email: formData.email.trim().toLowerCase(),
            territory: formData.territory || formData.state,
  ...
  const handleSubmit = (e) => {
    ...
    const otherEnquiries = (enquiries || []).filter(
      (enq) => enq.enquiryId !== formData.enquiryId && String(enq.id) !== String(id)
    );
    const existingDuplicate = checkDuplicateEnquiry(formData.email, formData.mobile, otherEnquiries);

    if (existingDuplicate) {
      setDuplicateMatch(existingDuplicate);
  ...
  return (
    <div className="enquiry-master-page container-fluid px-0 px-md-1">
      <Helmet>
        <title>{`Edit Enquiry (${formData.enquiryId})`} | Sonocare CRM</title>
        <meta name="description" content="Edit customer enquiry details in Sonocare CRM." />
      </Helmet>
      <ToastContainer />
      ...
      <div className="category-page-header mb-4">
        <div className="category-page-title-group">
          ...
          <h1 className="category-page-title">Edit Enquiry ({formData.enquiryId})</h1>
        </div>
      </div>
        ...
        {duplicateMatch && (
          <div className="py-2">
            ...
            <p className="small text-dark mb-0 fw-semibold">
              Please choose whether to cancel, view the existing enquiry, or continue updating this record anyway.
            </p>
          </div>
        )}
      </Modal>
